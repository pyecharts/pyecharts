...
DEFAULT_JS_LIBRARIES = dict(
    echarts="options.min",
    echartsgl="options-gl.min",
    liquidfill="options-liquidfill.min",
    wordcloud="options-wordcloud.min",
)
...
def test_core_js_libraries():
    __jupyter_echarts__ = conf.EXTENSION_MANAGER.get_a_extension("options")
...
def test_province_names():
    __provinces__ = conf.EXTENSION_MANAGER.get_a_extension("options-china-provinces-js")
...
def test_city_names():
    __cities__ = conf.EXTENSION_MANAGER.get_a_extension("options-china-cities-js")
